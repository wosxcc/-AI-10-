...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=False):
        ...
        # self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=(3,1), padding=(1,0), bias=False) #(3,1)
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1,  bias=False)  # (3,1)padding=1,
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        # self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=(1,5), padding=(0,2), bias=False)
        self.conv3 = nn.Conv2d(planes, planes * 4, kernel_size=1,  bias=False) #padding=(0, 2),
...
class SEResnet(nn.Module):
    ...
    def __init__(self, architecture):
        ...
        self.layer4 = self.make_layer(
            self.block, 512, self.layers[3], stride=1)
...
# from Alphapose_smoll.opt import opt
...
class FastPose(nn.Module):
    ...
    def __init__(self):
        ...
        self.duc2 = DUC(256, 512, upscale_factor=4)
        self.conv_out = nn.Conv2d(
            self.DIM, 2, kernel_size=3, stride=1, padding=1)
